...
# generate project
gum spin --title "Generating project" -- curl https://start.spring.io/starter.zip -d dependencies=web -d bootVersion=$VERSION -d type=$TYPE -d name=$PROJECT_NAME -o $PROJECT_NAME.zip && unzip -q $PROJECT_NAME.zip -d . && rm $PROJECT_NAME.zip
